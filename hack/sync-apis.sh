...
do_sync() {
    ...
    # Determine if we should use existing directory or clone
    local USE_EXISTING_DIR=false
    if [[ -n "${APIS_REPO_DIR}" ]] && [[ -d "${APIS_REPO_DIR}" ]] && [[ -d "${APIS_REPO_DIR}/.git" ]]; then
        USE_EXISTING_DIR=true
        print_info "Using existing git repository: ${APIS_REPO_DIR}"
        cd "${APIS_REPO_DIR}"
    ...
    elif [[ "${USE_EXISTING_DIR}" != "true" ]]; then
        # If directory was provided but doesn't exist or isn't a git repo, remove it
    ...
    # Clone repository if not using existing directory
    ...
    # If using existing directory (e.g., in GitHub Actions with peter-evans/create-pull-request),
    # don't create branch or commit - let the Action handle it
    if [[ "${USE_EXISTING_DIR}" == "true" ]]; then
        print_info "Repository is managed externally (e.g., by peter-evans/create-pull-request)"
    ...
    # Create branch and commit (for local/manual usage)
...
Source: https://github.com/volcano-sh/volcano/commit/${VOLCANO_SHA}"
    ...
    if [[ "${PUSH}" == "true" ]]; then
        ...
        # Ensure remote URL has authentication
        local CURRENT_REMOTE_URL=$(git remote get-url origin 2>/dev/null || echo "")
        local GH_TOKEN=${GH_TOKEN:-${GITHUB_TOKEN:-}}
        local NEED_AUTH_UPDATE=false
        local AUTH_URL=""

        # Check if current remote URL has authentication
        local HAS_AUTH=false
        if [[ "${CURRENT_REMOTE_URL}" =~ x-access-token: ]] || [[ "${CURRENT_REMOTE_URL}" =~ git@github.com ]]; then
            HAS_AUTH=true
        fi

        # If APIS_REPO_URL contains token, use it
        if [[ "${APIS_REPO_URL}" =~ x-access-token: ]]; then
            if [[ ! "${HAS_AUTH}" == "true" ]] || [[ "${CURRENT_REMOTE_URL}" != "${APIS_REPO_URL}" ]]; then
                AUTH_URL="${APIS_REPO_URL}"
                NEED_AUTH_UPDATE=true
            fi
        # If we have GH_TOKEN and remote doesn't have auth, build auth URL
        elif [[ -n "${GH_TOKEN}" ]] && [[ "${HAS_AUTH}" == "false" ]] && [[ "${APIS_REPO_URL}" =~ github.com ]]; then
        ...
        if [[ "${NEED_AUTH_UPDATE}" == "true" ]] && [[ -n "${AUTH_URL}" ]]; then
            print_info "Updating remote URL with authentication..."
            # Mask token in log output
            local AUTH_URL_DISPLAY="${AUTH_URL}"
            if [[ "${AUTH_URL_DISPLAY}" =~ x-access-token:([^@]+)@ ]]; then
                AUTH_URL_DISPLAY="${AUTH_URL_DISPLAY//x-access-token:[^@]*@/x-access-token:***@}"
            fi
            print_info "Setting remote URL to: ${AUTH_URL_DISPLAY}"
            git remote set-url origin "${AUTH_URL}"
        elif [[ "${HAS_AUTH}" == "false" ]] && [[ -z "${GH_TOKEN}" ]]; then
            print_warning "No authentication token found. Push may fail."
            print_info "Please set GH_TOKEN or GITHUB_TOKEN environment variable"
            print_info "Or use APIS_REPO_URL with token: https://x-access-token:TOKEN@github.com/..."
            print_info ""
            print_info "Example:"
            print_info "  export GH_TOKEN=your_github_token"
            print_info "  export APIS_REPO_URL=\"https://github.com/PersistentJZH/apis.git\""
        fi

        # Verify remote URL has authentication before pushing
        local FINAL_REMOTE_URL=$(git remote get-url origin 2>/dev/null || echo "")
        if [[ ! "${FINAL_REMOTE_URL}" =~ x-access-token: ]] && [[ ! "${FINAL_REMOTE_URL}" =~ git@github.com ]]; then
            print_warning "Remote URL does not have authentication. Push may fail."
            print_info "Current remote URL: ${FINAL_REMOTE_URL}"
            if [[ -z "${GH_TOKEN}" ]]; then
                print_error "GH_TOKEN or GITHUB_TOKEN is required for pushing"
                print_info "Set it with: export GH_TOKEN=your_token"
                exit 1
            fi
        fi
